  ...
  static fetchByCollectionId(id, queryParams = {}, isSync = false, type = 'samples', ElKlass) {
    ...
    switch (type) {
      ...
      case 'reactions':
        ...
        group = userState?.profile?.data?.filters?.reaction?.group || 'none';
        sort = userState?.profile?.data?.filters?.reaction?.sort || false;
        addQuery = `&sort_column=${(sort && group) || 'updated_at'}`;
        break;
      case 'generic_elements':
        userState = UserStore.getState();
        let filters = userState?.profile?.data?.filters || {};
